...
# (threshold, glyph, colour) ramp from darkest -> brightest: deep-purple shadows
# through violet to near-white highlights; empty space for the background.
RAMP = [
    (0.00, " ", None),
    (0.10, ".", "#241452"),
    (0.20, ":", "#3b1f7a"),
...
def build_neofetch_svg(art, cols, rows, cfg, utc_label):
    ...
    bg, dim = th["bg"], th["dim"]
    user = cfg["username"]
    # block 2 uses a white / red palette (distinct from the green calendar).
    # `green` here is the red accent; reusing the name keeps the diff small.
    green = "#ff7b72"   # accent: keys, section headers, prompt, scan line
    fg = "#f5d6d3"      # near-white (warm red tint) values
    white = "#ffece9"   # brightest highlight (name)
    blue = "#f85149"    # deep red for the prompt path glyph
    ...
    def stack_rows(items, per_row=5):
        rows = []
        for i in range(0, len(items), per_row):
            rows.append([("  " + " · ".join(items[i:i + per_row]), fg, False)])
        return rows
